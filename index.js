...
steam.on("loggedOn", async () => {
	...
	if (rank.rank_id < 7 || rank.wins < 150) {
		...
		if(tg_enabled === true){bot.sendMessage(chatid, "[" + steam_name + "] " + (rank.rank_id < 7 ? "Our rank is too low" : "We do not have enough wins") + " in order to request Overwatch cases. You need at least 150 wins and " + Translate("skillgroup_7") + ".");}
		steam.logOff();
		return;
	}
...
coordinator.on("receivedFromGC", async (msgType, payload) => {
	...
	if (body.reason === Helper.OverwatchConstants.EMMV2OverwatchCasesUpdateReason_t.k_EMMV2OverwatchCasesUpdateReason_Assign &&
		...
		if (!response || !response.ok) {
			...
			if(tg_enabled === true){bot.sendMessage(chatid, "[" + steam_name + "] Failed to download case. Check your internet connection");}
		...
		if (typeof config.parsing.forceConvictOnPreviousBan === "number" && config.parsing.forceConvictOnPreviousBan >= 0) {
			...
			if (typeof days !== "number") {
				...
				if(tg_enabled === true){bot.sendMessage(chatid, "[" + steam_name + "] Suspect has not been banned before");}
				...
				if(tg_enabled === true){bot.sendMessage(chatid, "[" + steam_name + "] Suspect has been banned " + days + " day" + (days === 1 ? "" : "s") + " ago, forcefully convicting");}
				...
				if(tg_enabled === true){bot.sendMessage(chatid, "[" + steam_name + "] Suspect has been banned " + days + " day" + (days === 1 ? "" : "s") + " ago, ban too old to convict");}
				data.forceConvicted = false;
			}
		} else {
			data.forceConvictEnabled = false;
		...
		if (timeTotal < (config.parsing.minimumTime * 1000)) {
			...
			if (minutes > 0) {
				...
				if(tg_enabled === true){bot.sendMessage(chatid, "[" + steam_name + "] Waiting " + minutes + ":" + seconds + " minutes before sending verdict...");}
			} else {
				console.log("Waiting " + seconds + " seconds before sending verdict...");
				if(tg_enabled === true){bot.sendMessage(chatid, "[" + steam_name + "] Waiting " + seconds + " seconds before sending verdict...");}
			}
